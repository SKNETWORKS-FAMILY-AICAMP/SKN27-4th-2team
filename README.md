...
사용자 질문
   |
   v
질문 분석 및 검색 질의 생성 (LangGraph Node)
   |
   v
PGVector 유사 문서 검색 (langchain_pg_embedding)
   |
   v
검색 문서와 사용자 컨텍스트 결합 (Prompt Formatting)
   |
   v
LLM 답변 생성 (OpenAI GPT Model)
   |
   v
출처와 함께 사용자에게 응답 제공
```
...
### 4.4.2. 로그인 사용자 대화 저장 및 기억 활용 흐름
